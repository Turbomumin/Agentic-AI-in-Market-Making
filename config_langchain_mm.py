...
# ===== LCMM: append the LangChain market maker =====
from agent.langchain_mm_agent import LangChainMarketMaker
from agent.ExchangeAgent import ExchangeAgent
import numpy as np
from pathlib import Path

# Find the exchange agent created above
exchange_agent_id = next(a.id for a in agents if isinstance(a, ExchangeAgent))
print(f"[LCMM] exchange_agent_id = {exchange_agent_id}", flush=True)

# Use the parsed ticker from args; don't probe 'symbols' dict keys
symbol_used = symbol
print(f"[LCMM] symbol = {symbol_used}", flush=True)

_next_id = (max(a.id for a in agents) + 1) if agents else 10000
label = "rmsc04_langchain"  # keep this consistent with your -l flag
metrics_path = str(Path("log") / label / "LCMM_metrics.csv")

lcmm = LangChainMarketMaker(
    id=_next_id,
    name="LCMM",
    symbol="ABM",           # or stock_symbols[0]
    starting_cash=1_000_000,
    exchange_id=exchange_agent_id,
    levels=10,              # depth you want
    subscription_freq="5s", # optional; 1s is fine to start
    wakeup_freq="5s",
    size=50,
...
# Register LCMM type (your fork uses a list here)
if "agentTypes" in globals() and isinstance(agentTypes, dict):
    agentTypes[lcmm.id] = type(lcmm)
elif "agent_types" in globals() and isinstance(agent_types, list):
    agent_types.append(type(lcmm))  # list → append, do NOT index by id
elif "agent_types" in globals() and isinstance(agent_types, dict):
    agent_types[lcmm.id] = type(lcmm)
else:
    print("[LCMM] WARN: no agentTypes/agent_types registry found; continuing")

# Keep count if your config uses it
try:
    agent_count += 1
except NameError:
    pass

print(f"[LCMM] appended agent id {lcmm.id}", flush=True)
# ================================================
...
latency_rstate = np.random.RandomState(seed=np.random.randint(low=0, high=(1<<31)-1))
